...
export default function SoupsNStewsPage() {
    ...
    useEffect(() => {
        fetchSoupsNStewsRecipeInfo()
    }, [])

    function fetchSoupsNStewsRecipeInfo() {
        const soupnstew = "soupnstew"
        API.getAllCategoryRecipe(soupnstew).then(data => {
            setRecipeInfoState({
                recipes: data
            })
        })
    ...
    return (
        <Container>
            <h1>Browse Soups/Stews Recipes!</h1>
            <InputGroup
                style={{
                    width: "250px",
                    marginLeft: "auto",
